...
function renderVehicleProfile(data) {
  ...
  if (scoreEl) {
    ...
    const dash = Math.round((s / 100) * 207);
    scoreEl.innerHTML = `
      <div class="score-wrap">
        <svg class="score-ring" viewBox="0 0 100 100" width="75" height="75">
          <circle cx="50" cy="50" r="33" fill="none" stroke="var(--surface-2)" stroke-width="8"/>
          <circle cx="50" cy="50" r="33" fill="none"
                  stroke="${ringColor}" stroke-width="8"
                  stroke-dasharray="${dash} 207"
                  stroke-dashoffset="0"
